  ...
  def process_message(bot, message)
    ...
    when Telegram::Bot::Types::Message
      ...
      when '/start', '/ayuda', '/help'
        text = @message_handler.render_happy_message('Puedes pedirme ayuda con lo siguiente:', self.get_help)
        reply(bot, message, text)
      when '/holi', '/holo'
        text = @message_handler.render_happy_message("Holi, #{message.from.first_name}.", nil)
        reply(bot, message,  text)
      when '/mis_grupos'
      ...
      when '/mejor_no'
        text = @message_handler.render_confused_message('Bueni.', nil)
        reply_and_clear_kb(bot, message, text) if require_private_chat(bot, message)
      when '/mi_zona'
...
Si quieres saber cómo funciono, o si quieres copiar mi código #{@message_handler.get_bashful}, puedes encontrarlo aquí: https://github.com/thebozzcl/SecretariaBot

Si tienes preguntas, necesitas ayuda o si me matas accidentalmente #{@message_handler.get_confused}, mándale un mensaje a @TheBozzUS diciendo 'Bozzolo, no funciona'."
  end

  def require_private_chat(bot, message)
    if message.chat.type != 'private'
      text = @message_handler.render_bashful_message('Nu, me da vergüenza... háblame en privado.', nil)
      reply(bot, message, text)
      return false
    end
    return true
